class MobileDevice:
    ...
    def search_internet(request: str) -> str:
        ...
        if not isinstance(request, str):
            raise TypeError("Запрос должен быть в виде строки!")
...
class SamsungMobile(MobileDevice):
    def __init__(self, model: str, screen_size: float, camera_resolution: int):
        ...
        if not isinstance(screen_size, float) or not isinstance(camera_resolution, int):
            raise TypeError("Размер экрана и разрешение камеры должны быть числами!")
        if camera_resolution <= 0 or screen_size <= 0:
            raise ValueError("Размер экрана и разрешение камеры должны быть положительными числами!")
    ...
    def screen_edge(mode: bool) -> str:
        ...
        if not isinstance(mode, bool):
            raise TypeError("Режим должен быть bool'евым значением!")
        if mode:
...
class DigitalPhone(MobileDevice):
    ...
    @staticmethod
    def search_internet(*args, **kwargs) -> str:
